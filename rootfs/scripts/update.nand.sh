...
FILE_USR="usr.yaffs2"
FILE_ETC="jffs2.yaffs2"
FILE_DTB="anyka_ev500.dtb"
FILE_MAC="mac.bin"
FILE_ENV="env.bin"
FILE_LOGO="anyka_logo.rgb"

PARTITION_KERNEL="KERBK"                                                        #实际写入的KERNEL备份分区
PARTITION_ROOT="ROOTBK"                                                         #实际写入的ROOT备份分区
PARTITION_USR="APPBK"                                                           #实际写入的APP备份分区
...
if [ "$?" = 0 ];then                                                            #没有找到分区表
    ...
    if [ "$?" = 0 ];then                                                        #没有找到升级文件
        ...
        updatecmd="${UPDATER} \
                    --use-linux-table \
                    --dir ${DIR_UPDATE} \
                    --file-partition ${FILE_KERNEL},${PARTITION_KERNEL},${ASA_KERNEL} \
                    --file-partition ${FILE_ROOT},${PARTITION_ROOT},${ASA_ROOT} \
                    --file-partition ${FILE_USR},${PARTITION_USR},${ASA_USR}"
    fi
else                                                                            #找到分区表使用分区表升级方式
